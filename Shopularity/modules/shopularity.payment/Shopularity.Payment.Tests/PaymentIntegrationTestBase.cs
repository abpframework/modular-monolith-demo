...
namespace Shopularity.Payment.Tests;

/* Inherit your integration test classes from this class */
public abstract class PaymentIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(PaymentModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(PaymentModule).Assembly.FullName} assembly!");
        }
