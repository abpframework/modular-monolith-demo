...
namespace Shopularity.Data.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Shopularity") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Shopularity", DisplayName = "Shopularity API", Resources = { "Shopularity" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Shopularity_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Shopularity_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // Public Client
        var publicClientId = configurationSection["Shopularity_Public:ClientId"];
        if (!publicClientId.IsNullOrWhiteSpace())
        {
            var webPublicRootUrl = configurationSection["Shopularity_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Shopularity_Public:ClientSecret"] ?? "1q2w3e*",
