...
namespace Shopularity.Ordering.Tests;

/* Inherit your integration test classes from this class */
public abstract class OrderingIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(OrderingModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(OrderingModule).Assembly.FullName} assembly!");
        }
